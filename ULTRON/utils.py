...
from ULTRON import CMD_LIST, LOAD_PLUG, LOGS, ULTRON, SUDO_LIST, ULTRON, bot
from ULTRON.Config import Config
from ULTRON.helpers.exceptions import CancelProcess
from var import Var
...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"ULTRON/packages/{shortname}.py")
        name = "ULTRON.packages.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        LOGS.info("ULTRON ~ " + shortname)
    else:
        import ULTRON.utils
        ...
        # support for uniborg
        sys.modules["uniborg.util"] = ULTRON.utils
        mod.Config = Config
        ...
        # support for PRO-ULTRON originals
        sys.modules["ULTRON.utils"] = ULTRON.utils
        sys.modules["ULTRON"] = ULTRON
        sys.modules["ULTRON.utils"] = ULTRON.utils
        sys.modules["ULTRON"] = ULTRON
        # support for paperplaneextended
        sys.modules["ULTRON.events"] = ULTRON.utils
        spec.loader.exec_module(mod)

        # for imports
        sys.modules["ULTRON.packages." + shortname] = mod
...
def start_assistant(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"ULTRON/packages/rishabhai/{shortname}.py")
        name = "ULTRON.packages.rishabhai.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        ...
        mod.tgbot = bot.tgbot
        spec.loader.exec_module(mod)
        sys.modules["ULTRON.packages.rishabhai" + shortname] = mod
...
def start_spam(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"ULTRON/packages/ultronspam/{shortname}.py")
        name = "ULTRON.packages.ultronspam.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        print("Starting тнαησѕ ultronspam Bot.")
        ...
        mod.tgbot = bot.tgbot
        spec.loader.exec_module(mod)
        sys.modules["ultronspam" + shortname] = mod
...
def load_addons(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"ULTRON/packages/Xtra_package/{shortname}.py")
        name = "ULTRON.packages.Xtra_package.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["Xtra_package." + shortname] = mod
        LOGS.info("💥тнαησѕ Extra Plugin💥 ~ " + shortname)
...
def load_abuse(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"ULTRON/packages/Abuse/{shortname}.py")
        name = "ULTRON.packages.Abuse.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
...
def remove_plugin(shortname):
    try:
        try:
            ...
            name = f"ULTRON.packages.{shortname}"
